...
class xenoichi(BaseBot):
    def __init__(self):
        ...
        self.valid_url_prefixes = ["https://on.soundcloud.com/", "https://soundcloud.com"] #Added valid prefixes
    ...
    messages_dict_dj = {
        "greeting1": "\n🎵 Включаем музыку!  Добро пожаловать к нашему DJ-боту!\n\n🎧 Закажи песню командой /play [название песни] или /linkplay [ссылка SoundCloud].\n",
        "greeting2": "\n🎶 Привет! \n\n🎵 Используй команду /play [название песни] или /linkplay [ссылка SoundCloud], чтобы добавить трек в очередь.\n",
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.startswith('/linkplay '): # search by link
            ...
            if not self.is_valid_url(song_request):
                await self.highrise.send_whisper(user.id, "Неверная ссылка, я могу только по SoundCloud")
                return
